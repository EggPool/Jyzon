...
class JSONRPCHandler(RequestHandler):
    ...
    async def prepare(self):
        ...
        if (username, password) != (
            self.interface.config.rpcuser,
            self.interface.config.rpcpassword,
        ):
            print("Auth failed")
            self.set_status(403)
...
def _get_error(exception):
    return {
        ...
        "message": getattr(exception, "message", str(exc_info()[0]))[
...
async def _get_response(request, interface, request_body):
    ...
    try:
        ...
        result = await _get_result(
            request, _get_method(interface, request_body), request_body.get("params")
        )
        if interface.config.verbose:
        ...
        return _get_with_protocol_version(
            {"id": request_id, "result": None, "error": _get_error(exception)}, version
        )

    if request_id:
        return _get_with_protocol_version(
            {"id": request_id, "result": result, "error": None}, version
        )
